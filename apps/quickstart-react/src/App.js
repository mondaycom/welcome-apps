    ...
    <div className="container" style={{ maxWidth: "800px", margin: "20px auto", padding: "20px" }}>
      ...
      <Box padding={Box.paddings.MEDIUM} margin={Box.marginBottoms.MEDIUM}>
        ...
        <div style={{ display: "grid", gap: "24px", gridTemplateColumns: "1fr 1fr" }}>
          <Box padding={Box.paddings.MEDIUM} border={Box.borders.DEFAULT} rounded={Box.roundeds.MEDIUM}>
            <Heading type={Heading.types.h3} value="V1 Storage (Instance-based)" />
            <div style={{ display: "flex", gap: "8px", margin: "16px 0" }}>
              <Button size={Button.sizes.MEDIUM} onClick={setItemV1} disabled={!key.trim()}>
                Set
              </Button>
              <Button size={Button.sizes.MEDIUM} onClick={getItemV1} disabled={!key.trim()} kind={Button.kinds.SECONDARY}>
                Get
              </Button>
              <Button size={Button.sizes.MEDIUM} onClick={deleteItemV1} disabled={!key.trim()} kind={Button.kinds.TERTIARY}>
                Delete
              </Button>
            </div>
            <div style={{ marginTop: "8px" }}>
              <strong>Stored Value:</strong> {storedValueV1 || "No value stored"}
            </div>
          </Box>

          <Box padding={Box.paddings.MEDIUM} border={Box.borders.DEFAULT} rounded={Box.roundeds.MEDIUM}>
            <Heading type={Heading.types.h3} value="V2 Storage (Instance-less)" />
            <div style={{ display: "flex", gap: "8px", margin: "16px 0" }}>
              <Button size={Button.sizes.MEDIUM} onClick={setItemV2} disabled={!key.trim()}>
                Set
              </Button>
              <Button size={Button.sizes.MEDIUM} onClick={getItemV2} disabled={!key.trim()} kind={Button.kinds.SECONDARY}>
                Get
              </Button>
              <Button size={Button.sizes.MEDIUM} onClick={deleteItemV2} disabled={!key.trim()} kind={Button.kinds.TERTIARY}>
                Delete
              </Button>
            </div>
            <div style={{ marginTop: "8px" }}>
              <strong>Stored Value:</strong> {storedValueV2 || "No value stored"}
            </div>
          </Box>
        </div>
      </Box>
