...
const createVerboseDetails = (data) => {
  ...
  if (data.rawValue !== undefined)
    verbose.rawValue = safeStringify(data.rawValue);
  if (data.extractedValue !== undefined)
    verbose.extractedValue = safeStringify(data.extractedValue);
  if (data.expectedValue !== undefined)
    verbose.expectedValue = safeStringify(data.expectedValue);
...
export const testAllStorageCapabilities = async (token) => {
  const testResults = {
    summary: {
      ...
      disclaimer:
        "All tests on the monday code backend using the apps-sdk is for GENERIC (V2) storage only",
    },
    testCases: [],
    errors: [],
  };
  ...
  try {
    ...
    try {
      ...
      const retrievedValue =
        getResult1 &&
        typeof getResult1 === "object" &&
        getResult1 !== null &&
        getResult1.value !== undefined
          ? getResult1.value
          : getResult1;
      ...
      try {
        ...
        if (deletedResult && deletedResult.success === false) {
          ...
          if (
            ...
            testResults.testCases.push({
              name: "Delete verification (get deleted item)",
              status: "FAIL",
              details: `Item still exists after deletion. Response: ${JSON.stringify(
                deletedResult
              )}`,
              verboseDetails: createVerboseDetails({
                request: { key: testKey1, operation: "get" },
        ...
        testResults.testCases.push({
          name: "Delete verification (get deleted item)",
          status: "PASS",
          details: `Delete verified - get threw expected error${
            is404 ? " (404)" : ""
    ...
    try {
      const setTtlResult = await storage.set(testKey2, testValue2, {
        ttl: ttlSeconds,
      });
      testResults.testCases.push({
        name: "Set with TTL",
      ...
      const ttlRetrievedValue =
        ttlResult &&
        typeof ttlResult === "object" &&
        ttlResult !== null &&
        ttlResult.value !== undefined
          ? ttlResult.value
          : ttlResult;
    ...
    const edgeTestData = [
      ...
      { key: "numbers", value: "1234567890", desc: "Numeric string" },
      {
        ...
        desc: "Mixed case and characters",
      },
    ];
    ...
    for (const testData of edgeTestData) {
      try {
        ...
        const hasNewlines = testData.key === "newlines";
        const isValidationCase =
          isWhitespaceOnly ||
          isEmptyString ||
          hasNewlines ||
          testData.isValidationCase;
        ...
        try {
          ...
          if (edgeRetrievedValue === testData.value) {
            ...
            isValidationCase &&
            (!edgeRetrievedValue ||
              edgeRetrievedValue === null ||
              edgeRetrievedValue === undefined ||
              (typeof edgeRetrievedValue === "string" &&
                edgeRetrievedValue.trim() === ""))
          ) {
            // Expected behavior: API may normalize or reject empty/whitespace/newline values
            let reason;
            if (isEmptyString) {
              reason = "Empty string normalized";
            } else if (isWhitespaceOnly) {
              reason = "Whitespace normalized";
            } else if (hasNewlines) {
              reason = "String with newlines normalized";
            } else {
            ...
            const expectedPreview =
              typeof testData.value === "string" && testData.value.length > 0
                ? testData.value.substring(0, 50)
                : String(testData.value).substring(0, 50);
            const gotPreview =
              edgeRetrievedValue &&
              typeof edgeRetrievedValue === "string" &&
              edgeRetrievedValue.length > 0
            ...
            testResults.testCases.push({
              ...
              verboseDetails: createVerboseDetails({
                ...
                valueLength: edgeRetrievedValue
                  ? typeof edgeRetrievedValue === "string"
                    ? edgeRetrievedValue.length
                    : "N/A"
                  : "null/undefined",
              }),
            });
    ...
    try {
      ...
      const concurrencyPromises = Array.from({ length: 10 }, (_, i) =>
        storage
          .set(`${baseKey}_${i}`, `Value ${i}`, { ttl: 300 })
      ...
      testResults.testCases.push({
        ...
        verboseDetails: createVerboseDetails({
          ...
          response: {
            ...
            rateLimited: concurrentRateLimits,
      ...
      if (concurrentSuccesses > 0) {
        ...
        testResults.testCases.push({
          ...
          verboseDetails: createVerboseDetails({
            request: {
              operation: "verify_concurrent_writes",
              baseKey: baseKey,
            },
            response: {
              verificationResults,
    ...
    try {
      ...
      try {
        ...
        const nonExistentValue =
          nonExistentResult &&
          typeof nonExistentResult === "object" &&
          nonExistentResult !== null &&
          nonExistentResult.value !== undefined
        ...
        if (nonExistentResult && nonExistentResult.success === false) {
          ...
          testResults.testCases.push({
            name: "Non-existent key handling",
            status: "FAIL",
            details: `Unexpected value returned for non-existent key: ${JSON.stringify(
              nonExistentValue
            )}`,
            verboseDetails: createVerboseDetails({
              request: { key: nonExistentKey, operation: "get" },
    ...
    try {
      ...
      for (const period of periods) {
        try {
          ...
          if (counterResult && counterResult.error) {
            ...
            counterResult &&
            typeof counterResult.newCounterValue === "number"
          ) {
            testResults.testCases.push({
              name: `Increment counter (${period})`,
              status: "PASS",
              details: `Successfully incremented. New value: ${
                counterResult.newCounterValue
              }, Message: ${counterResult.message || "N/A"}`,
              verboseDetails: createVerboseDetails({
                request: { period, operation: "incrementCounter" },
                response: counterResult,
              }),
      ...
      try {
        const counterWithOptions = await storage.incrementCounter("DAILY", {
          incrementBy: 2,
        });

        if (
          counterWithOptions &&
          typeof counterWithOptions.newCounterValue === "number"
        ) {
          testResults.testCases.push({
